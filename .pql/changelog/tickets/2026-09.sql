...
Adding the check without settling the second leaves the repo unable to state its own escaping rules in a doc comment, and a normalizing commit made in ignorance of it would bake the false statements in permanently rather than fixing them. Order matters here: decide the comment form, correct the three, then gate.', 'in_progress', 'high', NULL, NULL, NULL, '2026-08-16 11:42:52.797', '2026-09-09 10:15:08.787', NULL, '847225b4736dad24caeb0a706678a324', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06G0MRDXFNP425XTEVVMQB21X8', 'bug', NULL, 'Nothing in the gate checks formatting, so gofmt drift lands all at once', '`make fmt` writes. `make fmt-check` reads. Neither is reachable from any gate, so formatting drifts silently and lands all at once.
...
RESOLVED (2026-09-09). The two prerequisites the body ordered turned out to be already done: commit b9ff24b applied the normalizing sweep and corrected the three doc comments gofmt had broken (lex.go''s escaping doc now carries the digraph in an indented block, the gofmt-stable form; the fence comment reads correctly). What remained was the wiring, done where the body looked for it and found nothing: .golangci.yaml now has a formatters: section enabling gofmt, which golangci-lint v2 runs in check mode during run — so make lint, CI and pre-push all enforce it with no new stage, per D-33 (the script is the definition; the config travels with it). gofmt only, deliberately: the repo does not enforce import grouping, so goimports stays a write-time convenience in make fmt. Verified the gate bites: two lines of real struct-tag drift in decisions.go failed golangci-lint run with ''File is not properly formatted (gofmt)'' before make fmt cleared it. make fmt-check remains as the read-only listing tool.', 'done', 'high', NULL, NULL, NULL, '2026-08-16 11:42:52.797', '2026-09-09 10:16:54.437', NULL, '5831e854e26f5b8e11154e3a5335e997', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06FY7JHX6RH2BQK3R9VVP1ZX68', 'bug', '06G8AYX5XJWXT91BZHE2A8JYAG', 'a mutation against a populated changelog with an empty db silently relabels from T-1', 'Observed 2026-08-09 in pql''s own repo. Running ''pql --vault <repo> ticket new'' where .pql/pql.db does not exist but .pql/changelog/ holds a full history does NOT import the changelog first. It creates an empty db and mints the new ticket as T-1, colliding with the existing T-1. The vault already ran to T-94. Nothing is corrupted - identity is the ULID and the changelog is append-only - but the label collision is silent at creation time. It surfaced only on the next plan rebuild, which reported it correctly and clearly. Two contributing gaps. First, the docs say plan import runs automatically on a fresh clone; that holds only where pql init has planted the hooks. pql''s own repo has never been inited (doctor reports config.loaded false, no .pql/config.yaml), so nothing auto-imports there. Second, and the real fix: any planning mutation should detect changelog-present-but-db-empty and either import first or refuse with a diagnostic. Silently starting the label sequence at 1 is the one behaviour that produces a collision. Recovery, for the record: plan rebuild --verify restores everything and reports the collision, then relabel by record_id. Note relabel T-1 fails with a circular message telling you to run relabel; the ambiguous label cannot address either record, so the ULID is required. Worth a clearer diagnostic there too.
...
Nothing about the proposed fix changes. The guard still belongs in the mutation path rather than in the hooks, for the reason already given: the hooks are what is missing whenever this bites.', 'in_progress', 'high', NULL, NULL, NULL, '2026-08-08 23:51:49.814', '2026-09-09 10:19:19.559', NULL, '67a95e587ff2a980a24d6cf68e8d1653', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06FY7JHX6RH2BQK3R9VVP1ZX68', 'bug', '06G8AYX5XJWXT91BZHE2A8JYAG', 'a mutation against a populated changelog with an empty db silently relabels from T-1', 'Observed 2026-08-09 in pql''s own repo. Running ''pql --vault <repo> ticket new'' where .pql/pql.db does not exist but .pql/changelog/ holds a full history does NOT import the changelog first. It creates an empty db and mints the new ticket as T-1, colliding with the existing T-1. The vault already ran to T-94. Nothing is corrupted - identity is the ULID and the changelog is append-only - but the label collision is silent at creation time. It surfaced only on the next plan rebuild, which reported it correctly and clearly. Two contributing gaps. First, the docs say plan import runs automatically on a fresh clone; that holds only where pql init has planted the hooks. pql''s own repo has never been inited (doctor reports config.loaded false, no .pql/config.yaml), so nothing auto-imports there. Second, and the real fix: any planning mutation should detect changelog-present-but-db-empty and either import first or refuse with a diagnostic. Silently starting the label sequence at 1 is the one behaviour that produces a collision. Recovery, for the record: plan rebuild --verify restores everything and reports the collision, then relabel by record_id. Note relabel T-1 fails with a circular message telling you to run relabel; the ambiguous label cannot address either record, so the ULID is required. Worth a clearer diagnostic there too.
...
RESOLVED (2026-09-09). The guard the body asked for exists, in the mutation path as prescribed: changelog.GuardReplicaCurrent refuses when tickets is empty while .pql/changelog/tickets/ holds non-empty history — refuse rather than auto-import, per fail-unless-proven, and because auto-importing would paper over the missing hooks that cause the state. It lives in the changelog package so a future planning MCP inherits it; the CLI wraps it as openPlanningDBForMutation, used by all 12 ticket mutation verbs (new, append, status, relabel, assign, setparent, decision, block, unblock, team, label, refine write). Reads and the remedies (plan import/rebuild) are unguarded. Refusal is exit 65 with a hint naming pql plan import. Verified end-to-end on a scratch vault: create T-1, rm pql.db, ticket new refuses with the diagnostic; plan import then ticket new mints T-2 — the exact collision this ticket observed is unreachable. Unit tests cover fresh-vault pass, populated pass, the refusal, import-clears-it, and empty-changelog-files pass. The secondary observation (relabel T-1''s circular message when the ambiguous label cannot address either record) was not addressed here — it only occurs post-collision, which the guard now prevents from arising.', 'done', 'high', NULL, NULL, NULL, '2026-08-08 23:51:49.814', '2026-09-09 10:23:59.663', NULL, '3b1aef7c307e394c9893ec27fd4ce213', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06G64CE0WTT40RJ9TE0VT95FYW', 'task', '06G8AYX5XJWXT91BZHE2A8JYAG', 'ticket new allocates a label from a replica it never checks is current', 'Label collisions are detected at replay and repaired by `ticket relabel --fix-prose`. Nothing checks before the label is handed out, so the window between allocation and replay is where the damage happens.
...
This is the single-writer assumption showing at the seam rather than a bug in any one verb: with one clone, allocating from local state is always right and replay is a formality. With two, it is optimistic locking without the check, and the failure is silent.', 'in_progress', 'medium', NULL, NULL, NULL, '2026-09-02 12:57:02.950', '2026-09-09 10:25:00.311', NULL, '53ca40a514798612c7e4702389685827', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06G64CE0WTT40RJ9TE0VT95FYW', 'task', '06G8AYX5XJWXT91BZHE2A8JYAG', 'ticket new allocates a label from a replica it never checks is current', 'Label collisions are detected at replay and repaired by `ticket relabel --fix-prose`. Nothing checks before the label is handed out, so the window between allocation and replay is where the damage happens.
...
DECIDED (2026-09-09): D-35 settles the log-or-replica question this ticket raised — log, with the push boundary as the rewrite rule. Of the three shapes weighed here, the outcome is closest to the first, narrowed: not a force-full export, but a redact-class verb that rewrites the affected rows in pql.db, ticket_history (old_value copies included) and the changelog lines together, refusing when any affected line is already in a pushed commit. Pushed-history redaction stays a documented git-rewrite procedure, per the record. This ticket becomes the implementation carrier for that verb.', 'in_progress', 'medium', NULL, NULL, 'D-35', '2026-09-09 06:38:53.722', '2026-09-09 10:38:13.720', NULL, '85ab8eb1cde397ff42abb426504c6297', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06G89S0F3600NPD1978YNJP9CR', 'task', '06G8AYX5XJWXT91BZHE2A8JYAG', 'plan export cannot regenerate the changelog, so redacting ticket prose means rebuild-and-refile', 'Found 2026-09-09 while removing sibling-repo names and a hostname from a ticket
description before pushing. `make secrets` caught it, which is the system
working — CLAUDE.md is explicit that ticket prose is published prose, and the
gate ran before anything left the machine. What is missing is the route from
...
RESOLVED (2026-09-09). pql ticket redact <id> <value> <replacement> is the supported route this ticket asked for. Shape: closest to the second option weighed here (a redact verb scoped to prose, rewriting rows in place rather than appending), governed by D-35 rather than a force-full export — the changelog stays a log, and the push boundary is the rewrite rule. It reaches the tickets row, every ticket_history row including old_value copies (the fix-forward duplication this ticket documented), and the changelog lines carrying them, through the D-28 staged-SQLite path with hashes recomputed by the same canonical projection the db side uses — so untouched lines return byte-identical and the rewritten file replays with verifying hashes (tested: replay into a fresh replica, rehash, compare). The asymmetry stands by design: plan export still cannot regenerate the changelog, because regeneration is exactly the replica reading D-35 rejected; what redaction needed was never regeneration, only a rewrite that moves both sides together. Pushed history refuses with the documented git-rewrite procedure as the named remedy. Verified live: leak planted via ticket append, caught in both tickets/ and ticket_history/ files, redacted in one command, plan rebuild --verify reports 0 rows lost.', 'done', 'medium', NULL, NULL, 'D-35', '2026-09-09 06:38:53.722', '2026-09-09 10:43:30.331', NULL, 'f1764f91cddad0933ef15952c3c027cd', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06FZ4FHC4YQRSRC071QNEWM64G', 'bug', '06G8AYX5XJWXT91BZHE2A8JYAG', 'Scrubbing a changelog file does not survive the next mutation', 'A value removed from a committed changelog file is re-published by the next ticket mutation, because pql.db still holds the original row and the write-through export (D-23) re-derives the file from it.
...
Held back from 2.2.0 deliberately. It changes ten verbs'' output shapes with real regression surface, immediately after a release, and none of it is blocking anyone now that the current behaviour is written down. Stays ready rather than backlog — it is well-specified and can be picked up whenever, it just should not have been rushed alongside two behaviour changes.', 'in_progress', 'medium', NULL, NULL, 'D-30', '2026-08-08 18:44:39.529', '2026-09-09 10:45:17.540', NULL, '98bd6d5cebf6c80304ce35448f1dc829', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06FY5C83X7F5WDSXK11XWKM47C', 'task', NULL, 'converge mutation verb return shapes and document them', 'Implements D-30.
...
RESOLVED (2026-09-09). All six batch-capable verbs converge on one renderer (renderMutationReceipt): one record changed returns that record whole, several return the summary shape ticket label established — {ticket_ids, action, <value-key>: <applied>}, with the applied value keyed semantically (status, assigned_to, team, parent_id, decision_ref, label). ticket label itself gained the missing half: a single-id call now returns the whole record where it previously returned the summary regardless of count. status --force''s cascade receipt lists every ticket actually closed, not just the ids named. block/unblock keep their pair receipt, append and refine write keep their documented single-record shapes, new keeps its {id}. The skill''s ''not every batch verb has converged'' caveat is replaced by the rule stated plainly, and the integration test that pinned the old N-records cascade shape now asserts the summary and confirms state via show — which is the documented receipt workflow. Shape change ships in the next minor per this ticket''s own hold-back note.', 'done', 'medium', NULL, NULL, 'D-30', '2026-08-08 18:44:39.529', '2026-09-09 10:48:59.831', NULL, '1f02ef426f4486ec4a23163b9724a065', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06G8D25RXR5Q0116T1VQBKKV3C', 'task', NULL, 'self-update sends no auth and reports a bare 403, so a rate-limited host reads as an outage', 'Observed 2026-09-09, minutes after v2.4.0 published: ''pql self-update'' on a host whose anonymous GitHub API quota was exhausted exits 69 with only ''GitHub API returned 403''. The failure is loud, which is right, but the diagnosis is absent: nothing distinguishes rate-limiting from a permissions problem or an outage, and nothing suggests the fix. Two improvements, both small. (1) fetchLatestRelease (internal/cli/selfupdate.go) should send ''Authorization: Bearer'' from GITHUB_TOKEN or GH_TOKEN when either is set — an authenticated call has a 5000/hr quota, and hosts that use gh already have the token in the environment. The asset downloads go to a different host (objects.githubusercontent.com) and are not rate-limited the same way; only the API call needs it. (2) On 403, read the X-RateLimit-Remaining header: if 0, say so and include the reset time in the hint, rather than the bare status code. Same T-72 principle as ever: distinguish the failure a caller can fix from the one they cannot.', 'backlog', 'medium', NULL, NULL, NULL, '2026-09-09 14:18:22.318', '2026-09-09 14:18:22.318', NULL, '218094e1747d705eec6c11fbf7a0e8cf', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06GCTEGNA7CSH8H9VHM3KKC024', 'task', NULL, 'ticket new: accept the description from a file or stdin', NULL, 'backlog', 'medium', NULL, NULL, NULL, '2026-09-23 07:45:30.961', '2026-09-23 07:45:30.961', NULL, '1c85f79c46f9a7038c9e39c937e89ca3', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06GCTEGRSZ9911SRAF9RBDHWJ8', 'task', NULL, 'Batch ids: accept space-separated ids or say to use commas', NULL, 'backlog', 'medium', NULL, NULL, NULL, '2026-09-23 07:45:31.855', '2026-09-23 07:45:31.855', NULL, '84fd07b98c75259582957977a9110a3c', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06GCTEGW5RJJJ44ZGKJ8F1RFH4', 'bug', NULL, 'Unknown --decision/--parent fails with a raw FOREIGN KEY error', NULL, 'backlog', 'medium', NULL, NULL, NULL, '2026-09-23 07:45:32.718', '2026-09-23 07:45:32.718', NULL, '17e659878070f4fa03f9d0ecfc6ab6a5', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06GCTEGZ3106HDDASVEHMHCF9C', 'bug', NULL, 'Linked worktrees: planning DB and hook shims should resolve to the main checkout', NULL, 'backlog', 'medium', NULL, NULL, NULL, '2026-09-23 07:45:33.464', '2026-09-23 07:45:33.464', NULL, '678cf69b51962e25d314828cbac9b186', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06GCTEGNA7CSH8H9VHM3KKC024', 'task', NULL, 'ticket new: accept the description from a file or stdin', 'From a pql-driven session in a downstream repo, filing detailed tickets (multi-paragraph specs) took two calls every time: `pql ticket new <type> <title> --description "..."` cannot take a file, so long descriptions either go through shell quoting (fragile, and command substitution trips agent permission allowlists) or through a second `pql ticket append <id> --file <path>` call.
...
Acceptance: a ticket created with `--description-file` has exactly the file''s contents as its description (no trailing-newline drift); passing two description sources is a usage error; the changelog entry records the description like any other create.', 'backlog', 'medium', NULL, NULL, NULL, '2026-09-23 07:45:30.961', '2026-09-23 07:45:37.299', NULL, 'cc9bec480f4c65f8fa1540640056518e', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06GCTEGRSZ9911SRAF9RBDHWJ8', 'task', NULL, 'Batch ids: accept space-separated ids or say to use commas', 'Batch ids are comma-separated (`pql ticket status T-1,T-2 done`, `pql ticket show T-1,T-2`), but the natural first guess is space-separated. `pql ticket status T-592 T-593 review` fails with cobra''s generic `accepts 2 arg(s), received 3`, which says nothing about the comma syntax. Agents driving pql hit this repeatedly and fall back to one call per id (or a shell loop, which trips permission allowlists).
...
Acceptance: `pql ticket status T-1 T-2 done` either works or names the comma syntax in its error; same for `ticket show T-1 T-2`; covered by CLI tests.', 'backlog', 'medium', NULL, NULL, NULL, '2026-09-23 07:45:31.855', '2026-09-23 07:45:38.153', NULL, '624fc62c98ba94eba2bf9d4860aa6ac4', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06GCTEGW5RJJJ44ZGKJ8F1RFH4', 'bug', NULL, 'Unknown --decision/--parent fails with a raw FOREIGN KEY error', '`pql ticket new task "..." --decision D-110` where D-110 is written in the markdown but not yet synced into pql.db fails with the raw SQLite error:
...
Acceptance: each unknown-reference case names the missing id and suggests the fix; no raw `constraint failed` text reaches the user; covered by CLI tests.', 'backlog', 'medium', NULL, NULL, NULL, '2026-09-23 07:45:32.718', '2026-09-23 07:45:38.643', NULL, '6adefefa6e6e28103bd07d841a26cd3f', 2) ON CONFLICT(record_id) DO UPDATE SET type=excluded.type, parent_record_id=excluded.parent_record_id, title=excluded.title, description=excluded.description, status=excluded.status, priority=excluded.priority, assigned_to=excluded.assigned_to, team=excluded.team, decision_ref=excluded.decision_ref, updated_at=excluded.updated_at, deleted_at=excluded.deleted_at, hash=excluded.hash, canonical_version=excluded.canonical_version WHERE excluded.updated_at >= tickets.updated_at;
INSERT INTO tickets (record_id, type, parent_record_id, title, description, status, priority, assigned_to, team, decision_ref, created_at, updated_at, deleted_at, hash, canonical_version) VALUES ('06GCTEGZ3106HDDASVEHMHCF9C', 'bug', NULL, 'Linked worktrees: planning DB and hook shims should resolve to the main checkout', 'Follow-up to T-58. T-58 made a linked git worktree its own vault root, which is right for the markdown side: DQR edits in the worktree are what `decisions validate` should see. But the planning DB (`.pql/pql.db`) is gitignored, user-authored state, so a fresh worktree has none. Inside a worktree every planning read is empty ("ticket T-1 not found") and every write lands in a throwaway DB that no other checkout sees.
