...
- New `governance/` directory layout for D/Q/R records, per D-21
  (T-32 initiative, T-33–T-37 children):
  - Three per-type subdirectories — `governance/decisions/`,
    `governance/questions/`, `governance/rejected/` — replace the
    flat `decisions/` layout with its asymmetric `questions-X`
    prefix.
  - The parser infers record type from the immediate parent
    subdirectory and domain from the filename stem. Backward-compat:
    if the configured root contains the legacy flat layout (no
    type subdirs), parsing falls back to the pre-D-21 behaviour.
  - `dqr_dir` config knob (default `governance`) in
    `.pql/config.yaml` lets repos override the parent directory
    name without migrating.
  - `pql init` plants the parent + three type subdirs and writes
    `<dqr_root>/README.md` with a hand-edited prologue (mechanism
    description + recommended starter domains) plus an
    auto-generated records section delimited by
    `<!-- pql:records -->`. The records section is regenerated by
    `pql decisions sync` and by subsequent `pql init` runs.
- `pql decisions validate` separates structural errors from style
  warnings. New `--no-style` flag suppresses warnings (errors stay
  unconditional). `pql decisions sync` surfaces the same warnings
  on stderr as a teachable moment — drift becomes visible at the
  most common interaction point, not only on explicit validate
  runs.
- Style checks shipped: filename convention (lowercase + hyphenated)
  and subdirectory/heading-prefix consistency under the new layout.

### Changed

- This repo's own DQR tree migrated from `decisions/` to
  `governance/{decisions,questions,rejected}/`. Q-records (all
  currently architecture-domain) consolidated into
  `governance/questions/architecture.md`.
