...
- **Amendment (2026-08-07, v1.12.0, T-59):** The tie-break moves from content hash to **changelog position**. The emitted guard is now `WHERE excluded.updated_at >= target.updated_at`: a strictly newer row still wins, and an exact tie resolves in favour of whichever statement is applied later — which is the row appended later in the changelog, since replay walks tables, files and lines in sorted order. The hash stays on every row for content integrity and as the emission-order tiebreaker in the exporter's `ORDER BY`; it no longer decides recency. **Why:** the 2026-06-16 amendment below fixed the *write* side, but rows written before it keep second granularity forever, so ties remain — and hash order is arbitrary with respect to causality, so those ties can still resolve against the later mutation on every replay. Append order within a file *is* causal order, and it repairs history rather than only preventing new occurrences (settled-reach T-1057 lost a description for a month this way). **Cost:** convergence now depends on statement order rather than being order-free. In practice every replica holding the same merged file replays it identically, so they converge; two replicas that merge the same divergence independently could order it differently, which the hash tiebreaker avoided. Judged the right trade: the failure it prevents is observed and silent, the one it admits is rare and visible. The guard text is baked into already-committed changelog lines, so historical files need rewriting to carry the new rule — tracked by T-68, which also introduces an explicit changelog format version. Regression-guarded by `TestImport_EqualTimestampResolvesByAppendPosition`.
- **Amendment (2026-06-16, v1.10.4):** This record assumes millisecond-precision timestamps, but the implementation wrote `updated_at`/`created_at` via `datetime('now')` — **second** precision. Two mutations to one row within the same wall-clock second therefore tied on `updated_at` and fell through to the hash tiebreaker, which is unbiased but unrelated to recency. The visible symptom: a `ticket new` immediately followed by `ticket status` reverted on `plan rebuild` about one run in three, non-deterministically (the random `record_id` shuffles which content hash sorts higher). Writes now use `strftime('%Y-%m-%d %H:%M:%f','now')`, restoring the millisecond-collision boundary the guard was designed around; the hash tiebreaker is once again the sub-millisecond fallback this record describes. Precision-only — no `canonical_version` bump (the canonicalisation *rules* are unchanged). Regression-guarded by `TestIntegration_Rebuild_PreservesSameSecondStatusChange`.
...
### D-25: Parent-completion guard — a ticket can't close while children are open
- **Date:** 2026-06-09
- **Refines:** [D-14](#d-14-no-status-transition-enforcement-in-pql); builds on [D-24](#d-24-ticket-status-set-is-configurable-modelled-as-four-classes)
- **Decision:** A ticket cannot move to a **terminal** status (class `terminal`, per D-24) while it has at least one live, non-terminal **direct child**. `repo.SetStatus` enforces this in the core and returns a typed `*IncompleteChildrenError` listing the open children. A child counts as closed once it is itself terminal (`done` *or* `cancelled`), matching the unblocked-dependency semantics. The guard applies to *all* terminal statuses uniformly — closing and abandoning are both "closing a parent over unfinished work." The CLI exposes an escape hatch: `pql ticket status <id> <terminal> --force` **cascades** the same terminal status down the subtree — every not-yet-closed descendant is set bottom-up (deepest first, so each close satisfies the guard), then the ticket itself; already-closed descendants are left untouched, and the command output lists every ticket it closed. The check is on **direct** children only because the invariant composes: you can't have closed a child while *its* children were open, so a closed direct child implies a closed subtree going forward. Replay (`plan import`/`rebuild`) bypasses the guard — it replays historical rows via `ON CONFLICT`, not `SetStatus`, so pre-existing data that predates the rule is left as-is (no defensive migration).
- **Rationale:** "Done" on a parent should mean the work under it is actually finished; silently allowing a parent to close over open children makes status meaningless for roll-ups and dashboards. This is a *tree-integrity* invariant, not a workflow state machine, so it doesn't reverse D-14 — status *ordering* stays unrestricted (any status → any status), and non-terminal moves and childless tickets are never blocked. Enforcing in the core (not just the CLI) means every consumer — the CLI today, a planning-surface MCP tomorrow — inherits the guarantee; the `--force` cascade is the CLI's policy for the legitimate "abandon this whole branch" case, expressed as closing the subtree rather than punching a hole in the invariant.
- **Cost:** Closing a non-leaf ticket now requires its children be closed first (or `--force`). The cascade is non-atomic (one `SetStatus` per node); a mid-cascade failure leaves a partially-closed subtree, but re-running `--force` converges since already-closed nodes are skipped. The guard adds one `ChildrenOf` query per terminal transition.
- **Raised by:** User request to block tickets being closed while their children are incomplete.

### D-26: Ticket identity split — record_id (underwater) vs ticket_id (friendly label)
- **Date:** 2026-06-09
- **Supersedes the framing of:** [T-54 / D-?] loud-merge-warning approach to id collisions; **refines:** [D-15](#d-15-replication-via-per-table-monthly-sql-changelog-files), [D-16](#d-16-inline-lww-guards-plus-content-hash-plus-canonical-row-columns), [D-19](#d-19-no-alter-table--schema-lives-in-create-statements-only)
- **Decision:** A ticket's identity is a collision-proof **`record_id`** (a locally-generated ULID, `planning.NewRecordID`, stdlib-only) — the `tickets` primary key, the target of every structural reference (`parent_record_id`, `ticket_deps.blocker/blocked_record_id`, `ticket_history.ticket_record_id`, `ticket_labels.ticket_record_id`), the changelog key (`ON CONFLICT(record_id)`), and the canonical-hash identity. The friendly **`ticket_id`** (`T-NNN`) is demoted to a mutable label in a new **`ticket_idmap`** table (`record_id ↔ ticket_id`), allocated `max+1` locally and referenced by nothing structural. The public output `id` stays `T-NNN` (a `record_id` field is added alongside); the CLI still accepts `T-NNN` everywhere (resolved label→record_id at the edges). `CanonicalVersion` bumps 1→2. Collision detection becomes **duplicate-label detection** over `ticket_idmap` (a label mapping to ≥2 records), and the fix is `pql ticket relabel <id|record_id> [--new-label] [--fix-prose]` — a one-row mapping change plus an optional rewrite of whole-word mentions in the DQR markdown (structural refs need no rewrite; they key on record_id). Decisions/questions/rejected keep their human ids — tickets only.
- **Rationale:** `T-NNN` was overloaded as both global identity and human label, but it's allocated against the *local* changelog with no coordinator, so two clones mint the same one and replay's `ON CONFLICT(id)` silently merged unrelated tickets. T-54 made that *loud*; this removes the failure mode entirely — distinct clones get distinct record_ids, so the graph can never corrupt, and a label clash is a cheap, visible relabel rather than silent data loss. This is the surrogate-key + natural-key pattern (git oid + ref name); it fits pql's uncoordinated-distributed-clone replication model (D-15) by making identity safe by construction.
- **Cost:** A core-identity refactor touching the schema (PK + all FK columns renamed `*_record_id`), every repo query (label→record_id resolution + idmap joins), the changelog (new `ticket_idmap` table), hashing (CanonicalVersion 1→2), and the legacy snapshot. The CanonicalVersion bump means every row re-hashes, so existing changelogs must be migrated. Per D-19 there is no shipped migration runner: a one-off local `cmd/migrate-ids` (not in the `pql` binary) rewrites each of the 3 known vaults' committed changelogs old→new format; remotes pull the migrated changelog + new binary, then `rm .pql/pql.db && pql plan rebuild`. The old `T-NNN` may persist as prose in DQR records — surfaced (and optionally rewritten) by `relabel --fix-prose`, never silently.
- **Raised by:** User: "consider an underwater hashed id and a friendly label … create a mapping table to reconcile and still avoid collisions."

### D-27: Planning list verbs project — ticket list drops description by default
- **Date:** 2026-07-08
- **Refines:** [D-5](#d-5-cli-first-not-mcp) (output contract, "safe for agents"); references [D-22](#d-22-zero-results-returns-exit-0-no-distinct-no-match-code)
- **Decision:** The planning list verbs gain a shared projection surface. `--fields id,status,title` returns JSON rows containing only the named keys, in the requested order (unknown names exit `64` listing the valid set; `'*'` means all fields). `--oneline` emits a plain-text `id<TAB>status<TAB>title` index — an explicitly non-JSON human/terminal mode, like `git log --oneline`, composing only with `--limit`. And `pql ticket list`'s **default projection now omits `description`**: `--full` (or `--fields '*'`) opts back into whole rows, and naming `description` in `--fields` always returns it. `decisions list` gets `--fields`/`--oneline` for consistency but keeps whole rows as its default (its rows are light; bodies live in the DQR markdown). `ticket show` and every other record-level surface still return complete records. Implementation is a CLI-side helper (`render.Project` + `renderProjectedList`); the repo layer keeps returning full rows.
- **Rationale:** `ticket list` serialized all fields of every ticket, and the heaviest field dominated: measured on the two consumer vaults, `description` was ~81% of a 735 KB dump (clide, ~425 tickets) and pushed `--under` listings past agent tool-output budgets (settled-reach, 486 KB) — the harness truncated the output and the command was unusable as emitted, forcing `jq` re-projection and ad-hoc wrapper scripts downstream. The cheap, common query ("what's on the board / find by title") was paying for the expensive, rare one (read full bodies). Defaulting to the index matches what the dominant caller reads and cuts ~80% of the payload; `--fields` lets agents ask for exactly the columns they reason over (the corroborating consumer needed `id,type,parent_id,title,priority,status` — no fixed compact set would have fit, which is why projection beats a hardcoded summary shape). This is pure projection on existing verbs: no new store, no ranker change, JSON stays JSON on the default path.
- **Cost:** Behavior change to `ticket list`'s default shape — callers that read `description` from list output must pass `--full` or name it in `--fields` (a CHANGELOG compatibility note ships with the release). `--oneline` is a second sanctioned non-JSON stdout mode (after `ticket new --id-only`); it is opt-in and rejects `--pretty`/`--jsonl` with exit `64` so the JSON contract can't be violated accidentally. The projection round-trips rows through `encoding/json` maps in the CLI (bounded by list size; the repo query is unchanged, so the SQL-level win is left on the table until a measured need).
- **Raised by:** FR-1 in `feature-request.md` — clide dogfooding (2026-06-17) and settled-reach `/whats-next` (2026-07-07) independently hitting the same truncated-dump wall. Accepted as T-56.
