...
### Q-14: Corpus-derived reference time for the recency signal
- **Status:** Open
- **Question:** Should the recency signal's reference instant be derived from the corpus (e.g. the max mtime in the candidate batch) instead of the wall clock, making recency a pure function of the vault?
- **Context:** T-126 made the reference instant injectable — captured once per enrichment pass on `signal.Context`, so a batch is self-consistent and tests can pin it. What that deliberately did not change: the reference is still `time.Now()` in production, so the same query on an unchanged vault still scores differently next week, and D-11's max-normalization *widens* the spread as everything ages uniformly (for candidates below the max, `(raw-δ)/(raw_max-δ) < raw/raw_max`), until totals that sat close together swap. A corpus-derived reference (max mtime in the batch, or in the vault) removes wall-clock from ranking entirely: identical vault, identical ranking, forever. It is also a semantic change — "modified recently" becomes "modified recently *relative to the vault's newest file*", which behaves differently on a dormant vault (everything scores as fresh) — which is why T-126 left it to its own decision rather than folding it in. Also on the table from that ticket: letting a vault zero the recency weight in its profile (useful where mtime is an install artifact, e.g. a git checkout — see T-124's fixture evidence), which is additive and could land with either answer here.
