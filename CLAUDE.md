...
- **Two stores, two regimes.** `<vault>/.pql/index.db` is a pure cache — anything in it must be regenerable from the vault; on `schema_version` mismatch, drop and rebuild (no migration code). `<vault>/.pql/pql.db` is user-authored planning state. It has **no migration runner today** (D-19): schema is `CREATE TABLE IF NOT EXISTS` in `internal/planning/schema.go` with a `CanonicalVersion`, and pql.db is regenerated from the committed `.pql/changelog/` (D-15) rather than altered in place — recovery is `rm .pql/pql.db && pql plan rebuild`. `plan rebuild` only replays the changelog-backed tables (tickets and friends); decisions/decision_refs are markdown-sourced (D-8) and outside the changelog, so rebuild auto-runs the equivalent of `pql decisions sync` when it finds decisions empty against a populated DQR tree and reports it under `decisions_synced` — but a rebuild that finds decisions non-empty (e.g. after a branch switch that edited existing records' bodies) does not, so run `pql decisions sync` yourself in that case (T-106). Forward migrations are a planned step before third-party distribution (D-19 amendment, tracked by T-44); don't add a migration framework before then. See `governance/decisions/architecture.md (D-3 + D-19)`.
