...
- `governance/decisions/architecture.md (D-3 + D-19 + D-28)` — the cache (`index.db`) vs user-authored state (`pql.db`) split, and how pql.db evolves (shared forward-migration runner per D-28; see below). Read before touching anything that persists under `.pql/`.
...
- **Two stores, two regimes.** `<vault>/.pql/index.db` is a pure cache — anything in it must be regenerable from the vault; on `schema_version` mismatch, drop and rebuild (no migration code). `<vault>/.pql/pql.db` is user-authored planning state. Schema is `CREATE TABLE IF NOT EXISTS` in `internal/planning/schema.go` with a `CanonicalVersion` — no ALTER deltas today, and `rm .pql/pql.db && pql plan rebuild` (replaying the committed `.pql/changelog/`, D-15) remains the recovery for a database no forward step can reach. `plan rebuild` only replays the changelog-backed tables (tickets and friends); decisions/decision_refs are markdown-sourced (D-8) and outside the changelog, so rebuild auto-runs the equivalent of `pql decisions sync` when it finds decisions empty against a populated DQR tree and reports it under `decisions_synced` — but a rebuild that finds decisions non-empty (e.g. after a branch switch that edited existing records' bodies) does not, so run `pql decisions sync` yourself in that case (T-106). Forward migrations exist and are shared across pql's version axes (D-28, superseding D-19's no-runner clause): `internal/planning/migrate/` is the runner, the changelog-format axis carries the first real step, and the pql.db axis ships a `schema_migrations` ledger with an empty step list — adding a schema step is a one-entry change, not a framework to build. Upgrades run from the post-merge hook and `pql plan upgrade`; `plan import`/`plan rebuild` detect and report only. See `governance/decisions/architecture.md (D-3 + D-19 + D-28)`.
