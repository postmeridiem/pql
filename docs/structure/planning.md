...
| `pql ticket list [--status S] [--team T] [--assigned A] [--decision D] [--label L] [--under T-NNN] [--leaf] [--unblocked] [--fields F] [--full] [--oneline]` | List tickets. The default projection omits `description` (D-27) — `--full` or `--fields '*'` opts back into whole rows; `--fields id,status,title` returns exactly the named keys in that order; `--oneline` emits a plain `id<TAB>status<TAB>title` index (non-JSON). `--under` restricts to recursive descendants of a ticket; `--leaf` to childless tickets; `--unblocked` to tickets whose blockers have all reached a terminal status. Composed (`--under E --leaf --unblocked`), they are the batch complement to `pql plan whatsnext`. |
